...
const updateAnimal = (animals, cb) => {
  const updatedAnimals = [];
  animals.forEach(animal => {
    updatedAnimals.push(cb(animal));
  });
...
const sortMeetingsByDay = (arr) => {
  meetings.sort((a, b) => {
      if (map[a.dayOfWeek] > map[b.dayOfWeek] ){return 1}
      else if (map[a.dayOfWeek] < map[b.dayOfWeek] ){return -1}
      else return 0 
  });
  return arr
}
...
const sortSchedule = (arr) => {
  meetings.sort((a, b) => {
      let aTime = a.start - a.end 
      let bTime = b.start - b.end 
      if (map[a.dayOfWeek] > map[b.dayOfWeek] ){return 1}
      else if (map[a.dayOfWeek] < map[b.dayOfWeek] ){return -1}
      else  {
